...
export default function CameraButton({ onPhoto, onAddWithoutPhoto }: Props) {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 flex flex-col items-center gap-2 pb-[max(1rem,env(safe-area-inset-bottom))]">
      <button
        type="button"
        aria-label="Take a photo"
        onClick={() => inputRef.current?.click()}
        className="pointer-events-auto flex h-20 w-20 items-center justify-center rounded-full bg-accent text-white shadow-lg active:scale-95"
      >
      ...
      <button
        ...
        className="pointer-events-auto rounded-full border border-ink/15 bg-surface px-4 py-2 text-sm shadow-md"
      >
        Add without a photo
      </button>
    </div>
