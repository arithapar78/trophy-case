...
test('with AI off, choosing a photo makes no request to the AI function', async ({ page }) => {
  ...
  page.on('request', (req) => {
    if (req.url().includes('/api/')) aiRequests.push(req.url())
  })
...
test('at the limit the button is disabled but saving by hand still works', async ({ page }) => {
  // Pretend 10 uses just happened.
  await page.evaluate(() => {
    const now = Date.now()
    localStorage.setItem('trophy-case.ai-uses', JSON.stringify(Array.from({ length: 10 }, (_, i) => now - i * 1000)))
  })
  await page.getByTestId('camera-input').setInputFiles(photoFile())
  const sheet = page.getByRole('dialog', { name: 'New achievement' })
...
test('a request straight to the function without a photo is refused', async ({ request }) => {
  const res = await request.post('/api/read-photo', { data: { today: '2026-09-21' } })
  expect(res.status()).toBe(400)
  expect(await res.json()).toEqual({ ok: false, message: 'No photo was sent.' })
})
